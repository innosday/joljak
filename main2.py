...
def simpleMoter(entity:StepMoter,steps,delay,rotation=False):
...
def startpoint():
    while not xleft.digitalRead():
        simpleMoter(xmoter,40,0.001,True)
    #while not yleft.digitalRead():
        #simpleMoter(ymoter,40,0.001,True)
...
while True:
    ...
    if serching:
        if len(checkObject) <=0:
            ...
            print(select)
            startpoint()
            for _ in range(select["pos"][0]//10):
                if xright.digitalRead():
                    break
                simpleMoter(xmoter,10,0.08)
            for _ in range(select["pos"][1]//10):
                if yright.digitalRead():
                    break
                simpleMoter(ymoter,10,0.8)
